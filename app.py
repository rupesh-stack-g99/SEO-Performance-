...
# Dynamic GSC internal file selection based on common match type
...
if gsc_zip_file and ga_csv_file:
    try:
        ...
        with zipfile.ZipFile(gsc_zip_file) as z:
            ...
            # Look for the selected report type file inside the zip
            ...
            if matched_gsc_files:
                ...
                st.sidebar.success(f"✅ Found and loaded GSC '{matched_gsc_files[0]}'")
            else:
                # Fallback: grab the first CSV in the zip if exact match fails
                csv_files = [f for f in file_list if f.endswith('.csv') and not f.startswith('__MACOSX')]
                if csv_files:
                    with z.open(csv_files[0]) as f:
                        df_gsc = pd.read_csv(f)
                    st.sidebar.warning(f"⚠️ '{gsc_report_type}' not found. Loaded '{csv_files[0]}' instead.")
                else:
                    st.error("❌ No CSV files found inside the GSC ZIP archive.")

        # --- STEP 2: Process GA CSV File ---
        df_ga = pd.read_csv(ga_csv_file)
        ...
        if df_gsc is not None and df_ga is not None:
            ...
            # Handle potential encoding/data cleaning issues (like removing strings from numeric values if any)
            for df in [df_gsc, df_ga]:
                for col in df.columns:
                    if df[col].dtype == 'object' and col in ['Clicks', 'Impressions', 'Sessions', 'Conversions']:
            ...
            if join_column in df_gsc.columns and join_column in df_ga.columns:
                ...
                if merged_df.empty:
                    st.warning(f"⚠️ The files merged successfully, but resulted in 0 matching rows. Make sure the data in the '{join_column}' column format matches exactly in both files (e.g., full URLs vs relative paths).")
                else:
                ...
                st.write("**GSC Columns available:**", list(df_gsc.columns))
                st.write("**GA Columns available:**", list(df_ga.columns))

    except Exception as e:
        st.error(f"An unexpected error occurred while parsing the data: {e}")
else:
    st.info("💡 Please upload the **GSC ZIP file** and the **GA CSV file** in the sidebar to populate the dashboard.")
